...
static void YGNodelayoutImpl(
  ...
  const YGFlexDirection flexColumnDirection =
    ...
    //获得设置的边框样式，：margin border padding
  const YGEdge startEdge = direction == YGDirectionLTR ? YGEdgeLeft : YGEdgeRight;
...
YOGA_EXPORT void YGNodeCalculateLayoutWithContext(
  ...
  node->resolveDimension();//获取css设置的样式，存入resolvedDimensions
  float width = YGUndefined;
    /**
     *确定owner 容器的空间尺寸，和计算模式：
     *1.YGMeasureModeExactly,能够确定自身的size
     *2.YGMeasureModeAtMost. 无法确定size，但是存在最大值约束
     *3.使用跟随父容器的宽度来计算。
     */
